...
__git_ssh_remote='git@github.com:arpanrec/dotfiles.git'
...
echo "-----------------------------------------------------------------------------------------------------------------------"
echo ""
echo "         DotStrap Init - Tracking dotfiles directly with Git"
echo ""
echo "-----------------------------------------------------------------------------------------------------------------------"
echo "-----------------------------------------------------------------------------------------------------------------------"
echo ""
...
if [[ "${__redownload_dotfiles}" == "Y" || "${__redownload_dotfiles}" == "y" ]]; then
    ...
    echo ""
    echo "Current git remote is pointing to https remote ${__git_remote}"
    echo ""
    git --git-dir="${__gitbare_directory}" --work-tree="${HOME}" remote -v
    echo ""
    read -n1 -p "Enter \"y\" to set ssh git remote ${__git_ssh_remote} (Press any other key to Skip*) : " __set_ssh_remote
    echo ""
    if [[ "${__set_ssh_remote}" == "Y" || "${__set_ssh_remote}" == "y" ]]; then
        git --git-dir="${__gitbare_directory}" --work-tree="${HOME}" remote set-url origin "${__git_ssh_remote}"
...
if [[ "${__git_checkout_setup}" == "Y" || "${__git_checkout_setup}" == "y" ]]; then
    ...
    echo ""
    echo "#######################################################################################################################################################"
    echo "Automated setup scripts available at ${__setup_script_dir}"
    echo "#######################################################################################################################################################"
    echo ""


    if [[ -f "${__setup_script_dir}/dotstrap/apply.sh" ]]; then
        echo ""
        echo "#######################################################################################################################################################"
        echo "Proceed with Automation"
        echo "#######################################################################################################################################################"
        echo ""
        ${__setup_script_dir}/dotstrap/apply.sh
        echo ""
        echo "#######################################################################################################################################################"
        echo "Completed"
        echo "#######################################################################################################################################################"
...
echo "-----------------------------------------------------------------------------------------------------------------------"
echo ""
echo "          DotStrap Completed - Tracking dotfiles directly with Git"
echo ""
echo "-----------------------------------------------------------------------------------------------------------------------"
echo "-----------------------------------------------------------------------------------------------------------------------"
echo ""
